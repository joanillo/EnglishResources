        ...
        if (xmlhttp.readyState==4 && xmlhttp.status==200) {
			...
			if (arr_dades.length == 0) {
				...
				for (var i = 0; i < arr_dades.length; i++) {
					...
					if (typeof (arr_dades[i].id_listenning) == "string") type = "L";
					p1.innerHTML = "<a href=\"" + arr_dades[i].url + "\" target=\"_blank\" class=\"btn btn-primary stretched-link\">" + arr_dades[i].id_resource + "</a> (" + type + ") " + arr_dades[i].resource;
