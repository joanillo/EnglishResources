...
for (var i = 0; i < arr_dades.length; i++) {
	...
	if (typeof (arr_dades[i].id_video) == "string") {
		content = content + "(V)&nbsp;";
		if (arr_dades[i].VS == "1") has = has + "S";
		if (arr_dades[i].VG == "1") has = has + ((has=="")?"G":" - G");
		if (arr_dades[i].VQ == "1") has = has + ((has=="")?"Q":" - Q");
		if (arr_dades[i].VV == "1") has = has + ((has=="")?"V":" - V");
	}

	if (typeof (arr_dades[i].id_listenning) == "string") {
		content = content + "(L)&nbsp;";
		if (arr_dades[i].LS == "1") has = has + "S";
		if (arr_dades[i].LG == "1") has = has + ((has=="")?"G":" - G");
		if (arr_dades[i].LQ == "1") has = has + ((has=="")?"Q":" - Q");
		if (arr_dades[i].lV == "1") has = has + ((has=="")?"V":" - V");
	}

	content = content + arr_dades[i].resource;

	if (typeof (arr_dades[i].description) == "string") {
		if (arr_dades[i].description!="") content = content + "<br />" + "Description: " + arr_dades[i].description;
	}

	if (typeof (arr_dades[i].comments) == "string") {
		if (arr_dades[i].comments!="") content = content + "<br />" + "Comments: " + arr_dades[i].comments;
	}
